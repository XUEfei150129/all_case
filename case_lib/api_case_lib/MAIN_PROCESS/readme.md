...
接口自动化用例，原则是是互不依赖，单独执行。但是目前业务场景的需要，需要我们按照场景将流程串联起来，实现不同的业务场景。场景的实现是覆盖更多的接口，在不同的前置条件下，检查接口的调用状态是否正常。

> 原则：将每个接口的入参，利用前一个接口或者其他接口的返回值，作为本接口的入参。避免因数据的差异，影响接口的调用。
